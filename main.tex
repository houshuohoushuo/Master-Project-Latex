...
\thesistitle{Classification of Breast Cancer Images} % Your thesis title, print it elsewhere with \ttitle
...
\vspace{6cm}
\begin{center}
Classification of Breast Cancer Images Using Deep Convolutional Neural Networks\end{center}
%\vspace*{\fill}
...
This project assesses the classification performance of three deep neural networks on the breast cancer specimen dataset provided by \citeauthor{Kosik2019} \citep{Kosik2019}. Three neural networks are trained on selected samples from the dataset, a small neural network derived from \citeauthor{Dhindsa2018}'s study \citep{Dhindsa2018}, and two state-of-art deep convolution neural networks VGG \citep{vgg} and ResNet \citep{resnet}. Three neural network models are trained on Ultrasound (US) images and on Photoacoustic tomography (PAT) images. Their performance is compared by accuracy and $F_1$ score. The neural network model and training process are parameterized using YAML for intuitive parameter fine tuning, also retraining when additional training data is available.\clearpage
%----------------------------------------------------------------------------------------
%	ACKNOWLEDGEMENTS
%----------------------------------------------------------------------------------------
